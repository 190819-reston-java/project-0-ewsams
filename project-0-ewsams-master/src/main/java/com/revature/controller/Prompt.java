...
import com.revature.exception.InvalidInput;
...
import com.revature.service.CheckingAccount;
import com.revature.service.SavingsAccount;
...
public class Prompt {
	 ...
	 int userInput;
	 public static Logger bankLogger = Logger.getLogger(Prompt.class);
	 Scanner customerChoice = new Scanner(System.in);
	 public Connection connection = ConnectionUtility.getConnection();
	 int optionNotRecognizedCounter = 0;
	 BankingCustomerDoa Doa = new BankingCustomerDoa();
	...
	private void makeWithdrawl() throws Exception {
		if(Prompt.loggedIn) {
			...
			for(int i=0; i<availableAccounts.size();i++) {
				if(Prompt.account == availableAccounts.get(i).getAccount().getAccountNumber()){
						double amount = 0;
				try {
					...
					Doa.updateAccount(availableAccounts.get(i));
		...
		if(availableAccounts.size()<=0) {
			...
			openAccount();
					...
					for(int i=0; i<availableAccounts.size();i++) {
						...
						Prompt.account = availableAccounts.get(i).getAccount().getAccountNumber();
